...
**Development status:** Piattro **0.2.0** implements a shared-profile portable
...
`./install` runs `piattro doctor`, prepares a pinned release from this checkout,
activates it, and symlinks `pi` and `piattro` into `~/.local/bin` (or
...
Piattro **0.2.0** uses one canonical user profile at `~/.piattro/agent`
(override with `PIATTRO_HOME`). It is seeded **once** on first activation from
...
theme-only package. Original Piattro code is [MIT](LICENSE); upstream notices
