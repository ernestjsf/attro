...
Piattro **0.2.0** (`agentMode: shared-v1`) uses one canonical user profile and
...
bin/piattro setup --repo /path/to/checkout   # manual prepare
bin/piattro update --repo /path/to/checkout
...
Launchers symlink this checkout's `bin/pi` and `bin/piattro`; keep the checkout
...
Global `--state-root PATH` (or `PIATTRO_HOME`) selects the managed directory,
defaulting to `~/.piattro`. If you set a custom `PIATTRO_HOME` for installation,
export the same value in every shell that runs `pi` or `piattro`. Piattro refuses
...
## Shared profile (`~/.piattro/agent`)

On first activation of a shared-profile release, Piattro seeds
`~/.piattro/agent` once from reviewed defaults in `profile/agent/` and pristine
...
Piattro **never copies or symlinks** live OAuth tokens, auth files, sessions,
...
| `PIATTRO_RESOURCE_DIR` | `<release>/profile/resources` |

Provider API keys and other ambient provider configuration from the environment
are intentionally inherited. Piattro does not override `HOME` or change
...
`PIATTRO_MANAGED_RESOURCE_ARGV` for downstream consumers.
...
legacy layout. Piattro refuses shared-v1 operations on v0.1 manifests and
...
`piattro try` uses a temporary agent directory seeded only from the release's
...
`piattro update --repo PATH` and `piattro activate RELEASE_ID` instead. This is
