...
FAKE_PIATTRO = '''#!/usr/bin/env python3
import fcntl
import json
import os
import sys
...
state_root = Path(os.environ["PIATTRO_HOME"])
...
if args[0] == "setup":
    ...
    release_id = os.environ.get("FIXTURE_RELEASE_ID", "piattro-0.1.0-fixture")
...
class InstallTests(unittest.TestCase):
    def setUp(self):
        ...
        (bin_root / "piattro").write_text(FAKE_PIATTRO)
    ...
    def launcher_targets(self) -> dict[str, Path]:
        return {name: (self.repo / "bin" / name).resolve() for name in ("pi", "piattro")}
    ...
    def test_success_creates_links_and_activates(self):
        ...
        self.assertEqual(state["active"], "piattro-0.1.0-fixture")
    ...
    def test_identity_matching_rerun_is_idempotent(self):
        self.run_install()
        before = {name: (self.bin_dir / name).readlink() for name in ("pi", "piattro")}
        self.run_install(extra_env={"FIXTURE_RELEASE_ID": "piattro-0.2.0-fixture"})
        after = {name: (self.bin_dir / name).readlink() for name in ("pi", "piattro")}
        self.assertEqual(before, after)
        self.assertEqual(json.loads((self.managed / "state.json").read_text())["active"], "piattro-0.2.0-fixture")
...
'''
    ...
    def test_launcher_lock_covers_activation(self):
        self.run_install(extra_env={"FIXTURE_EXPECT_BIN_LOCK": str(self.bin_dir / ".piattro-install.lock")})

    def test_symlinked_launcher_lock_is_rejected(self):
        self.bin_dir.mkdir()
        sentinel = self.base / "unrelated-file"
        sentinel.write_text("preserve")
        (self.bin_dir / ".piattro-install.lock").symlink_to(sentinel)
    ...
    def test_printed_environment_exports_preserve_literal_paths(self):
        ...
        self.env["PIATTRO_HOME"] = str(self.managed)
        result = self.run_install()
        ...
        home, path = shell.stdout.splitlines()
    ...
    def test_failed_activation_preserves_existing_links_and_active_release(self):
        self.run_install()
        self.run_install(extra_env={"FIXTURE_ACTIVATE_FAIL": "1", "FIXTURE_RELEASE_ID": "piattro-0.2.0-fixture"}, success=False)
        self.assertEqual(json.loads((self.managed / "state.json").read_text())["active"], "piattro-0.1.0-fixture")
        for name in ("pi", "piattro"):
